...
must_haves:
  ...
  artifacts:
    - path: "src/exports/edl.nim"
      ...
      exports: ["exportCMX3600EDL", "formatTimecode", "EDLClip"]
  key_links:
    - from: "src/exports/edl.nim"
      to: "src/cmds/clips.nim (06-04)"
      via: "EDLClip DTO consumed by CLI, Clip->EDLClip conversion in 06-04"
      pattern: "EDLClip.*startMs.*endMs.*engagementScore"
---
...
2. EDL clip type (self-contained DTO for EDL/JSON export - Clip->EDLClip conversion happens in 06-04 CLI):
```nim
type
  EDLClip* = object
